...
# SAP Fiori Horizon Theme Styling & Full UI Cleaner
st.markdown("""
    <style>
    .stApp {
...
if page == "Dashboard & Claims":
    ...
    if submitted:
        if not uploaded_files:
        ...
        elif not receipt_number.strip():
            st.error("❌ Petty Cash Slip / Receipt Number is mandatory.")
        elif amount <= 0:
            st.error("❌ Please enter a valid Amount.")
        elif not merchant.strip() or not description.strip():
            ...
            if existing_check and existing_check.data:
                ...
                if not OCR_AVAILABLE or amount_matched or not extracted_receipt_rows:
                    try:
                        uploaded_urls = []
    ...
    if st.session_state.user_role in ["Manager", "Admin", "Super Admin", "PPM", "GM"] or (st.session_state.username and st.session_state.username.lower() == "kapil"):
        df = fetch_records()
        if not df.empty:
    ...
    st.markdown("<div class='main-header'>📥 Financial Reports & Data Export</div>", unsafe_allow_html=True)
    df = fetch_records()
    
    if not df.empty:
        if not is_admin_or_kapil and assigned_prism:
            if 'prism_id' in df.columns:
                df = df[df['prism_id'] == assigned_prism]

        st.markdown("### 📊 Operational Summary Pivot Table")
        piv_col1, piv_col2 = st.columns(2)
        
        with piv_col1:
            if 'property_name' in df.columns and 'amount' in df.columns:
                prop_summary = df.groupby('property_name')['amount'].agg(['count', 'sum']).reset_index()
                prop_summary.columns = ['Property Name', 'Total Claims', 'Total Expense (EUR/GBP)']
                st.markdown("**Expenses by Property**")
                st.dataframe(prop_summary, use_container_width=True)

        with piv_col2:
            if 'category' in df.columns and 'amount' in df.columns:
                cat_summary = df.groupby('category')['amount'].agg(['count', 'sum']).reset_index()
                cat_summary.columns = ['Category', 'Total Claims', 'Total Expense (EUR/GBP)']
        ...
        col_exp1.download_button(
            ...
            data=csv_data,
            file_name=f"PRISM_Petty_Cash_Export_{date.today().strftime('%Y%m%d')}.csv",
            mime="text/csv"
        )
        
        buffer = io.BytesIO()
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            df.to_excel(writer, index=False, sheet_name='Petty Cash Claims')
        buffer.seek(0)
        
        col_exp2.download_button(
            label="📊 Download Excel Workbook (.xlsx)",
            data=buffer,
            file_name=f"PRISM_Petty_Cash_Export_{date.today().strftime('%Y%m%d')}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
    else:
        st.info("No records available to generate reports or export.")
